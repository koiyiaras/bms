...
if (!is_dir($options['outpath']) OR !is_writable($options['outpath'])) {
	echo "ERROR: Can't write to ".esc_html($options['outpath'])."\n\n";
	exit(3);
}

echo "\n>>> Converting fonts for TCPDF:\n";

echo '*** Output dir set to '.esc_html($options['outpath'])."\n";
...
foreach ($options['fonts'] as $font) {
	...
	if ($fontname === false) {
		$errors = true;
		echo "--- ERROR: can't add ".esc_html($font)."\n";
	} else {
		echo "+++ OK   : ".esc_html($fontfile).' added as '.esc_html($fontname)."\n";
	}
}
